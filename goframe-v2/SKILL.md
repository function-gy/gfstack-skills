...
- Follow the official `logic + service interface` architecture by default:
  - Put concrete business implementations in `internal/logic/<module>`.
  - Expose cross-module/business capabilities through interfaces in `internal/service`, generated by `gf gen service` where appropriate.
  - Business modules should depend on `service` interfaces instead of importing other `logic` packages directly.
  - Keep tightly related business changes in the same `logic/<module>` package to preserve module cohesion.
- Use `gf gen service` after creating or changing exported methods on logic service structs. See [Module Service Generation - gen service](./references/开发工具/代码生成-gen/模块规范-gen%20service.md) for details.
